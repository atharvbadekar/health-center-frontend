...
export default function DoctorPortal() {
  ...
  const handleSendOtp = async (e) => {
    ...
    // ------------------------------------------------------------------------
    // Validate mobile number for Staff / Other
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // Clear old patient details before new verification
    // ------------------------------------------------------------------------
    ...
    try {
      ...
      if (
        ...
        const returnedPatient =
          response.data.patientDetails || null;

        // --------------------------------------------------------------------
        // IMPORTANT:
        // Store the verified patient returned by backend.
        // --------------------------------------------------------------------
        ...
        if (isStudent) {
          ...
          if (!returnedPatient) {
            setError(
              'Student was found but patient details were not returned by the server.'
            );
          ...
          if (
            !returnedPatient.college_id
          ) {
            setError(
              'Verified student does not have a valid College ID.'
            );
          ...
          if (
            !returnedPatient.email
          ) {
            setError(
              'This student does not have an email address registered in the database.'
            );
  ...
  const handleVerifyOtp = (e) => {
    ...
    if (
      ...
      // ----------------------------------------------------------------------
      // IMPORTANT:
      // For students, make sure verified patient details still exist.
      // ----------------------------------------------------------------------

      if (
        isStudent &&
        (!patientDetails ||
          !patientDetails.college_id ||
          !patientDetails.email)
      ) {

        setError(
          'Patient verification data is missing. Please start again.'
        );
      ...
      if (isStudent) {
        ...
        console.log(
          '🎓 Verified student College ID:',
          patientDetails.college_id
        );

        console.log(
          '📧 Consultation email will be sent to:',
          patientDetails.email
        );
      ...
      setError(
        'Invalid OTP code. Please verify the 4-digit code.'
      );
  ...
  const handleSubmitConsultation =
    async (e) => {
      ...
      if (!selectedDoctor) {
        ...
        setError(
          'Please select an attending doctor.'
        );
      ...
      // ----------------------------------------------------------------------
      // IMPORTANT:
      // For students, DO NOT use the input field as the final patient ID.
      //
      // Use the College ID returned by the backend after successful lookup.
      // This prevents sending a consultation against a changed/wrong ID.
      // ----------------------------------------------------------------------
      ...
      if (isStudent) {
        ...
        if (
          ...
          setError(
            'Verified student details are missing. Please verify the student again.'
          );
      ...
      if (
        ...
        setError(
          'Please enter presenting symptoms.'
        );
      ...
      if (
        ...
        setError(
          'Please enter diagnosis / clinical treatment.'
        );
      ...
      if (
        ...
        setError(
          'Please enter prescription and dosage.'
        );
      ...
      const payload = {
        ...
        // IMPORTANT:
        // This is now the verified College ID.
        patient_id:
          patientId,
      ...
      if (isStudent) {
        ...
        console.log(
          'Student Email from DB:',
          patientDetails?.email
        );

        console.log(
          'Student College ID from DB:',
          patientDetails?.college_id
        );
      ...
      try {
        ...
        if (
          ...
          console.log(
            '📧 Email dispatched:',
            res.data.emailDispatched
          );

          console.log(
            '📧 Email target:',
            res.data.targetEmail
          );

          // --------------------------------------------------------------
          // IMPORTANT:
          // If this is a student and email was not dispatched,
          ...
          if (
            ...
            setError(
              `Consultation was saved, but the email could not be sent to ${patientDetails?.email || 'the registered student email'}.`
            );
            ...
            >
              ...
              <div className="bg-teal-50 border border-teal-200 p-4 rounded-xl text-center">
                ...
                <p className="text-xs text-teal-800 font-medium">
                  ...
                  OTP dispatched to registered contact.

                </p>

                <p className="font-mono font-black mt-2 text-2xl tracking-widest text-teal-950">
                  ...
                  {patientDetails ? (
                    ...
                    <p className="text-xs text-blue-900 font-semibold mt-0.5">
                      ...
                      <span className="text-slate-800 font-bold">
                        ...
                        {patientDetails.hostel_name || 'N/A'}
          ...
          {step === 4 && (
            ...
            <div className="text-center py-6 animate-in fade-in duration-300">
              ...
              <button
                ...
                className="py-3 px-6 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl transition font-bold text-xs"
              >
